            ...
            .then((res) => {
                ...
                if (res["data"]["playerstats"]["success"] == false) { // Private profile
                    if (res["data"]["playerstats"]["error"] == "Requested app has no stats")
                        $ctrl.pstatus = "The game \"Portal 2\" has not been purchased on steam account \"chase.rawlins\". Contact your local hunt admin if you wish to change the target steam profile, or if this information is innacurate.";
                    else if (res["data"]["playerstats"]["error"] == "Profile is not public")
                        $ctrl.pstatus = "Steam account \"chase.rawlins\" is private. To fix, go to your steam profile -> edit profile -> privacy settings -> change every option to public (especially game details). Contact your local hunt admin if you wish to change the target steam profile, or if this information is innacurate.";
                    else {
